IF OBJECT_ID('dbo.sp_analisis_precios_unitarios') IS NOT NULL
BEGIN
	DROP PROCEDURE dbo.sp_analisis_precios_unitarios
	IF OBJECT_ID('dbo.sp_analisis_precios_unitarios') IS NOT NULL
		PRINT '<<< FAILED DROPPING PROCEDURE dbo.sp_analisis_precios_unitarios>>>'
	ELSE
		PRINT '<<< DROPPED PROCEDURE dbo.sp_analisis_precios_unitarios >>>'
END
GO
...
CREATE PROCEDURE sp_analisis_precios_unitarios
(
	@operacion			VARCHAR(5)				
)
WITH ENCRYPTION
...
AS
	...
	IF @operacion = 'C1'					--> Consulta datos principales APU
	BEGIN
	...
	IF @operacion = 'C2'					--> Consulta datos de equipo
	BEGIN
	...
	IF @operacion = 'C3'					--> Reporte materiales
	BEGIN
	...
	IF @operacion = 'C4'					--> Reporte transporte materiales
	BEGIN
	...
	IF @operacion = 'C5'					--> Reporte mano de obra
	BEGIN
...
IF OBJECT_ID('dbo.sp_analisis_precios_unitarios') IS NOT NULL
	PRINT '<<< CREATED PROCEDURE dbo.sp_analisis_precios_unitarios >>>'
ELSE
	PRINT '<<< FAILED CREATING PROCEDURE dbo.sp_analisis_precios_unitarios >>>'
GO