IF OBJECT_ID('dbo.sp_calculo_del_AIU') IS NOT NULL
BEGIN
	DROP PROCEDURE dbo.sp_calculo_del_AIU
	IF OBJECT_ID('dbo.sp_calculo_del_AIU') IS NOT NULL
		PRINT '<<< FAILED DROPPING PROCEDURE dbo.sp_calculo_del_AIU>>>'
	ELSE
		PRINT '<<< DROPPED PROCEDURE dbo.sp_calculo_del_AIU >>>'
END
GO
...
CREATE PROCEDURE sp_calculo_del_AIU
(
	@operacion			VARCHAR(5)				
)
WITH ENCRYPTION
...
AS
	...
	IF @operacion = 'C1'					--> Reporte inicial
	BEGIN
	...
	IF @operacion = 'C2'					--> Reporte Gastos en el campo y oficinas
	BEGIN
	...
	IF @operacion = 'C3'					--> Reporte Gastos legales
	BEGIN
	...
	IF @operacion = 'C4'					--> Reporte Gastos de personal
	BEGIN
	...
	IF @operacion = 'C5'					--> Reporte Impuestos
	BEGIN
	...
	IF @operacion = 'C6'					--> Reporte Admisión imprevistos y utilidades
	BEGIN
...
IF OBJECT_ID('dbo.sp_calculo_del_AIU') IS NOT NULL
	PRINT '<<< CREATED PROCEDURE dbo.sp_calculo_del_AIU >>>'
ELSE
	PRINT '<<< FAILED CREATING PROCEDURE dbo.sp_calculo_del_AIU >>>'
GO